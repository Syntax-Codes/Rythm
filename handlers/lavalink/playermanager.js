...
module.exports = async (client, message, args, type, channel, guild) => {
  ...
  if (method[0] === "play")
    ...
    return message.channel.send(new MessageEmbed()
      ...
      .setTitle("❌ Error | No valid search Term? ... Please Contact: `SyntaxError The Dev 💜#3044`")
    );
}
...
const search = args.join(" ");
  ...
  if(message.guild.me.permissionsIn(message.channel).has("EMBED_LINKS")){
    ...
    .setAuthor(message.author.username, message.author.displayAvatarURL({dynamic: true}), "https://bit.ly/Web_Syntax")
    .setColor(ee.color)
  ...
  const search = args.join(" ");
    ...
    function song_() {
      ...
      //otherwise
          ...
          //create info msg embed
          let playembed = new Discord.MessageEmbed()
            .setAuthor(`Added to queue`, message.author.displayAvatarURL({dynamic: true}), "https://bit.ly/Web_Syntax")
  ...
  function playlist_() {
    ...
    var time = 0;
      let playlistembed = new Discord.MessageEmbed()
        ...
        .setAuthor(`Playlist added to Queue`, message.author.displayAvatarURL({dynamic:true}), "https://bit.ly/Web_Syntax" )
        .setColor(ee.color)
        .setTitle("**"+res.playlist.name+"**")
        .setThumbnail(`https://img.youtube.com/vi/${res.tracks[0].identifier}/mqdefault.jpg`)
          //timing for estimated time creation
